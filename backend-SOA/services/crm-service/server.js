...
const { PORT = 5007, MONGO_URL = "mongodb://127.0.0.1:27017/hrm-crm" } = process.env;
...
app.use("/crm", (_req, res, next) => {
  if (dbReady) return next();
  return res.status(503).json({
    message:
      "CRM database chưa sẵn sàng. Hãy khởi động MongoDB và cấu hình MONGO_URL (ưu tiên 127.0.0.1 trên Windows)."
  });
});

// Keep prefix consistent with gateway: gateway proxies /crm -> service handles /crm/*
app.use("/crm/customers", customersRouter);
...
async function connectWithRetry() {
  ...
  try {
    ...
    dbReady = false;
    // eslint-disable-next-line no-console
    console.error("CRM database connection failed:", err?.message || err);
