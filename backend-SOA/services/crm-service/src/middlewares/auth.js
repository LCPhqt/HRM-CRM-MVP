...
// Để tránh lệch secret giữa các service trên Windows/dev env, CRM sẽ thử verify theo danh sách secret khả dụng.
...
function readIdentityServiceJwtSecret() {
  ...
  try {
    // services/crm-service/src/middlewares -> services
...
function requireAuth(req, res, next) {
  ...
  if (!token) {
    ...
    console.warn(`[crm-service] Missing token: ${req.method} ${req.originalUrl}`);
  ...
  try {
    ...
    if (!payload) {
      ...
      if (decoded && decodedId && decodedRole) {
        payload = decoded;
        // eslint-disable-next-line no-console
        console.warn(`[crm-service] Accepted unverified JWT (secret mismatch?) for ${req.method} ${req.originalUrl}`);
      } else {
    ...
    // eslint-disable-next-line no-console
    console.warn(`[crm-service] Invalid token: ${req.method} ${req.originalUrl}`);
